...
if ( ! class_exists( 'WC_Email_Customer_Order_Proposal' ) ) {
	...
	*
	* @class 		WC_Email_Customer_Order_Proposal
	* @version		2.0.0
	* @package		WooCommerce/Classes/Emails
	* @author 		WooThemes
	* @extends 		WC_Email
	*/
	class WC_Email_Customer_Order_Proposal extends WC_Email {
		...
		*/
		function __construct() {
			...
			$this->id 				= 'order_proposal';
			$this->title 			= __( 'Customer Proposal Backend', 'woocommerce-order-proposal' );
			$this->description		= __( 'This is an order notification sent to customers containing their order proposal when created in the backend.', 'woocommerce-order-proposal' );

			$this->customer_email   = true;

			$this->template_base    = wc_order_proposal()->get_plugin_path() . "/templates/";
			$this->template_html 	= 'emails/customer-order-proposal.php';
			$this->template_plain 	= 'emails/plain/customer-order-proposal.php';
			...
			add_action( 'woocommerce_order_status_order-proposalreq_to_order-proposal_notification', array( $this, 'trigger' ), 10, 2 );
...
return new WC_Email_Customer_Order_Proposal();