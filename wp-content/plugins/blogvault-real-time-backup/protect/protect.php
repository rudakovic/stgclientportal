...
if (!class_exists('BVProtect_V602')) :
...
class BVProtect_V602 {
	...
	public static function init($mode) {
		...
		if ($mode == BVProtect_V602::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = BVProtectUtils_V602::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(BVProtect_V602::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new BVProtectRequest_V602($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			BVProtectFW_V602::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			BVProtectFW_V602::getInstance($mode, $request, $fw_config, $brand_name)->init();
			BVProtectLP_V602::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		BVProtectIpstore_V602::uninstall();
		BVProtectFW_V602::uninstall();
		BVProtectLP_V602::uninstall();

		BVProtect_V602::removeWPPrepend();
		BVProtect_V602::removePHPPrepend();
		BVProtect_V602::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		BVProtectUtils_V602::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		BVProtect_V602::removeHtaccessPrepend();
		BVProtect_V602::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		BVProtectUtils_V602::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		BVProtectUtils_V602::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		BVProtectUtils_V602::rrmdir($mc_data_dir);
	}
}
endif;