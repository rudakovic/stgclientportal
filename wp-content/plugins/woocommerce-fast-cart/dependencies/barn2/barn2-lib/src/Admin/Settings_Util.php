...
namespace Barn2\Plugin\WC_Fast_Cart\Dependencies\Lib\Admin;
...
class Settings_Util
{
    ...
    public static function get_help_links(Plugin $plugin)
    {
         ...
         * @since 1.1
         */
        $links = \apply_filters('barn2_plugin_settings_help_links', ['doc' => ['url' => $plugin->get_documentation_url(), 'label' => __('Documentation', 'wc-fast-cart'), 'target' => '_blank'], 'support' => ['url' => $plugin->get_support_url(), 'label' => __('Support', 'wc-fast-cart'), 'target' => '_blank']], $plugin);
