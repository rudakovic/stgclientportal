 ...
 * Calls the class.
 */
function sliced_call_subscription_invoices_updater_class() {
    return Sliced_Subscription_Invoices_Updater::get_instance();
}
add_action('sliced_loaded', 'sliced_call_subscription_invoices_updater_class', 1);


class Sliced_Subscription_Invoices_Updater {

	private $store_url   = 'http://slicedinvoices.com';
	private $name        = 'Subscription Invoices Extension';
	private $download_id = '14125';
	private $version     = SLICED_INVOICES_SUBSCRIPTIONS_VERSION;
	private $slug        = 'subscription_invoices';

	private $key_name    = 'subscription_invoices_license_key';
	private $status_name = 'subscription_invoices_license_status';
	private $error_name  = 'subscription_invoices_license_error';
	...
	public function plugin_updater() {
		// setup the updater
		new Sliced_Plugin_Updater( $this->store_url, SLICED_INVOICES_SUBSCRIPTIONS_FILE, array(
	...
	public function license_field( $options ) {
		$options['fields'][] = array(
			'name'        => $this->name,
			'desc'        => __( 'Enter the License Key for this extension', 'sliced-invoices-subscriptions' ),
			'id'          => $this->key_name,
			'type'        => 'text',
	...
	public function license_field_render_row_cb( $field_args, $field ) {
		...
		<div class="cmb-row cmb-type-text cmb2-id-<?php echo str_replace( '_', '-', $this->slug ); ?>-license-key table-layout">
			...
			<div class="cmb-td">
				...
				<p class="cmb2-metabox-description"><?php _e( 'Enter the License Key for this extension', 'sliced-invoices-subscriptions' ); ?></p>
	...
	public function after_field( $args, $field ) {
		...
		if ( $this->license_status ) {
			...
			$status = '<span class="license-status license-'. $status .'">' . sprintf( esc_html__( 'License: %s', 'sliced-invoices-subscriptions' ), $status ) . '</span>';
		}
		...
		$label = 'valid' === $this->license_status
			? esc_html__( 'Deactivate License', 'sliced-invoices-subscriptions' )
			: esc_html__( 'Activate License', 'sliced-invoices-subscriptions' );
			...
			if ( ! $this->license_key > '' ) {
				...
				if ( ! Sliced_Admin_Notices::has_notice( $this->slug . '_license_missing' ) ) {
					...
					$notice_args = array(
						...
						'content' => '<p>Sliced Invoices ' . $this->name . ': ' . sprintf( __( 'No license key found.  Don\'t forget to enter your license key on the <a href="%s">Licenses page</a>.', 'sliced-invoices-subscriptions' ), admin_url( 'admin.php?page=sliced_licenses' ) ) . '</p>',
						'dismissable' => true,
				...
				if ( ! Sliced_Admin_Notices::has_notice( $this->slug . '_license_expired' ) ) {
					...
					$notice_args = array(
						'class' => 'notice-error',
						'content' => '<p>' . sprintf( __( 'Your license for %s has expired.  Please <a href="%s">renew your license</a> to continue receiving updates and support.', 'sliced-invoices-subscriptions' ), 'Sliced Invoices ' . $this->name, 'https://slicedinvoices.com/checkout/?edd_license_key='.$this->license_key.'&download_id='.$this->download_id ) . '</p>',
						'dismissable' => true,
						'dismiss_transient' => '2592000',
					);
				...
				if ( ! Sliced_Admin_Notices::has_notice( $this->slug . '_license_disabled' ) ) {
					...
					$notice_args = array(
						'class' => 'notice-error',
						'content' => '<p>' . sprintf( __( 'Your license for %s has been disabled.  Please <a href="%s">contact us</a> for assistance.', 'sliced-invoices-subscriptions' ), 'Sliced Invoices ' . $this->name, 'https://slicedinvoices.com/contact-us/' ) . '</p>',
						'dismissable' => false,
					);
				...
				if ( ! Sliced_Admin_Notices::has_notice( $this->slug . '_license_inactive' ) ) {
					...
					$notice_args = array(
						'class' => 'notice-warning',
						'content' => '<p>' . sprintf( __( 'Your license for %s is not active.  Be sure to activate your license key on the <a href="%s">Licenses page</a> to receive updates and support.', 'sliced-invoices-subscriptions' ), 'Sliced Invoices ' . $this->name, admin_url( 'admin.php?page=sliced_licenses' ) ) . '</p>',
						'dismissable' => true,
						'dismiss_transient' => '2592000',
					);
