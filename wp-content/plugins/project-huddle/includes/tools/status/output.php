...
Redirects
---------
Canonical Redirects : <?php echo has_filter( 'template_redirect', 'redirect_canonical' ) ? 'Good' : 'Disabled: Please enable canonical redirects for SureFeedback to work properly.'; ?>
...
foreach( $plugins as $plugin_path => $plugin ) {
	if( !in_array( $plugin_path, $active_plugins ) )
		...
		echo (isset($plugin['Name']) ? $plugin['Name'] : '') . ': ' . (isset($plugin['Version']) ? $plugin['Version'] : '') . ' ' . (isset($plugin['Author']) ? $plugin['Author'] : '') . ' ' . (isset($plugin['PluginURI']) ? $plugin['PluginURI'] : '') . "\n";
}
