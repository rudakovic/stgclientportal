...
<?php
	...
	public function display()
	{
		...
		$url_headers = false;
		$headers = __( 'Unable to fetch headers', 'project-huddle' );

		try {
			$url_headers = @get_headers(esc_url($test_url), 1);
		} catch (Exception $e) {
			$headers = 'Error fetching headers: ' . $e->getMessage();
		}

		$headers = 'Security Headers Okay';
		if (is_array($url_headers) || is_object($url_headers)) {
			foreach ($url_headers as $key => $value) {
				if ('X-Frame-Options' === $key) {
					$headers  = 'Not OK';
					$headers .= ' - currently set to ' . $value . '.';
					$headers .= ' You need to remove security headers to use SureFeedback on external sites.';
				}
			}
		}
