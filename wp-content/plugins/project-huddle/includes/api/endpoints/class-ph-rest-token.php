...
class PH_REST_Token
{
    ...
    public function init()
    {
        ...
        add_action('init', array($this, 'add_cors_support')); // Move CORS header call earlier.
    ...
    public function add_cors_support()
    {
        if ( ! headers_sent() ) { // Ensure headers aren't already sent.
            ...
            header(sprintf('Access-Control-Allow-Headers: %s', $headers)); // Remove '@' to avoid suppression.
        }
    }
    ...
    public function generate_token(WP_REST_Request $request)
    {
        if (!$this->secret_key) {
            return new WP_Error(
                ...
                __('The SureFeedback REST API is not configured correctly. Please contact support.', 'project-huddle'),
                array(
                    'status' => 500,
                )
            );
