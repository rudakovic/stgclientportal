...
class PH_REST_Users_Controller extends WP_REST_Controller
{
	...
	public function prepare_item_for_response($user, $request)
	{
		...
		if (!empty($schema['properties']['roles'])) {
			...
			$data['roles'] = array_values( $user_roles );

			if ( ! empty( $user_roles ) ) {
				$user_role = reset( $user_roles ); // Get the first role assigned to the user
                $translated_role = isset( $wp_roles->roles[ $user_role ]['name'] ) ? translate_user_role( $wp_roles->roles[ $user_role ]['name'] ) : '';

				$data['user_role'] = $translated_role;
			}
		}
		...
		$context = $this->get_user_response_context($user, $request);
	...
	/**
 * Determines the response context based on user capabilities.
 *
 * @param WP_User         $user
 * @param WP_REST_Request $request
 * @return string
 */
protected function get_user_response_context($user, $request)
{
	if (user_can($user, 'administrator') || user_can($user, 'project_admin') || user_can($user, 'project_editor')) {
		return 'edit';
	}

	return !empty($request['context']) ? $request['context'] : 'embed';
}
