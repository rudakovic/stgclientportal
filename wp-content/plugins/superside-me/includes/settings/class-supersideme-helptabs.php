...
class SuperSide_Me_HelpTabs extends SuperSide_Me_Helper {
		...
		switch ( $active_tab ) {
			...
			case 'optional':
				$help_tabs = array(
					...
					array(
						'id'      => 'supersideme-widget-help',
						'title'   => __( 'Widget Location', 'superside-me' ),
						'content' => wp_kses_post( $this->widget() ),
					),
				);
				break;
			...
			case 'appearance':
				$help_tabs = array(
					...
					array(
						'id'      => 'supersideme-icons-help',
						'title'   => __( 'Custom Icons', 'superside-me' ),
						'content' => wp_kses_post( $this->icons() ),
					),
				);
				break;
	...
	/**
	 * Instructions for custom icons.
	 * @return string
	 */
	protected function icons() {
		/* translators: link to the Font Awesome website. */
		$help = '<p>' . sprintf( __( 'If you are using SVG icons, you can easily change them. SuperSide Me includes <a href="%s" target="_blank" rel="noopener">Font Awesome 5 icons (free only)</a> by default.', 'superside-me' ), esc_url( 'https://fontawesome.com/icons?d=gallery&s=brands,solid&m=free' ) ) . '</p>';
		/* translators: links to a tutorial on using the filter */
		$help .= '<p>' . sprintf( __( 'If you want to use your own SVG icons, keep this setting checked and type in the SVG names as needed. You may want to <a href="%s" target="_blank" rel="noopener">use a filter to disable loading the SuperSide Me SVG</a>.', 'superside-me' ), esc_url( 'https://robincornett.com/docs/theme-svg-supersideme/' ) ) . '</p>';
		$help .= '<p>' . __( 'If you have used SuperSide Me before 2.4.0, SVG icons are turned off by default for backwards compatibility.', 'superside-me' ) . ' ';
		/* translators: link to options tab, where the Font Awesome settings are. */
		$help .= sprintf( __( 'If you choose to switch to SVG, you may also want to <a href="%s">disable SuperSide Me from loading Font Awesome</a> (this will not happen automatically due to instructions in past tutorials).', 'superside-me' ), esc_url( admin_url( '/themes.php?page=supersideme&tab=optional' ) ) ) . '</p>';
	...
	/**
	 * Instructions for the widget location setting.
	 * @return string
	 */
	protected function widget() {
		return '<p>' . __( 'You may choose to place the SuperSide Me widget either at the end of the menu panel (default), or before the panel menus/after the search input.', 'superside-me' ) . '</p>';
	}
}