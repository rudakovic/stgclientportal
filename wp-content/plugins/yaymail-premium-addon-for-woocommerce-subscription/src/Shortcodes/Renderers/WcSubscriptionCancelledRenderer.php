...
/**
 * @method: static WcSubscriptionCancelledRenderer get_instance()
 */
class WcSubscriptionCancelledRenderer {
    ...
    public function initialize_titles() {
            $this->titles = [
                ...
                'price_date'  => $this->element_data['price_title'] ?? TemplateHelpers::get_content_as_placeholder( 'price_title', esc_html__( 'Price', 'yaymail' ), $this->is_placeholder ),
                ...
                'end_prepaid' => $this->element_data['end_prepaid_title'] ?? TemplateHelpers::get_content_as_placeholder( 'end_prepaid_title', esc_html__( 'End of Prepaid Term', 'yaymail' ), $this->is_placeholder ),
            ];
    }
    ...
    private function initialize_sample_data() {
        $this->sample_subscription = [
            'id'          => 1,
            'price_date'  => wc_price( 10 ),
            'last_date'   => gmdate( 'm-d-Y' ),
            'end_prepaid' => gmdate( 'm-d-Y' ),
        ];
    }
    ...
    public function render() {
        ...
        ?>
        <table class="yaymail-wc-subscription-cancelled td" cellspacing="0" cellpadding="6" style="<?php echo esc_attr( $table_style ); ?>" border="1" width="100%">

            <thead>
                <tr>
                    <th class="td id-title" scope="col" style="<?php echo esc_attr( $style ); ?>"><?php echo esc_html( $this->titles['id'] ); ?></th>
                    <th class="td price-date-title" scope="col" style="<?php echo esc_attr( $style ); ?>"><?php echo esc_html( $this->titles['price_date'] ); ?></th>
                    <th class="td last-date-title" scope="col" style="<?php echo esc_attr( $style ); ?>"><?php echo esc_html( $this->titles['last_date'] ); ?></th>
                    <th class="td end-prepaid-title" scope="col" style="<?php echo esc_attr( $style ); ?>"><?php echo esc_html( $this->titles['end_prepaid'] ); ?></th>
                </tr>
            </thead>
            ...
            <tbody>
                    <?php
                        ...
                        $subscription_price_date = $is_using_sample ? $this->sample_subscription['price_date'] : $this->subscription->get_formatted_order_total();
                        ...
                        $subscription_end_prepaid = $is_using_sample ? $this->sample_subscription['end_prepaid'] : date_i18n( wc_date_format(), $this->subscription->get_time( 'end', 'site' ) );
                    ?>
                <tr>
                    ...
                    <td class="td price-date-row" scope="row" style="<?php echo esc_attr( $style ); ?>">
                        <?php echo wp_kses_post( $subscription_price_date ); ?>
                    </td>
                    <td class="td last-date-row" scope="row" style="<?php echo esc_attr( $style ); ?>">
                        <?php echo wp_kses_post( $subscription_last_date ); ?>
                    </td>
                    <td class="td end-prepaid-row" scope="row" style="<?php echo esc_attr( $style ); ?>">
                        <?php echo esc_html( $subscription_end_prepaid ); ?>
                    </td>
                </tr>
            </tbody>
        </table>
