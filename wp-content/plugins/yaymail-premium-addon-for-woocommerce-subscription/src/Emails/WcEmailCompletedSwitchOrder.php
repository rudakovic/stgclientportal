...
/**
 * WcEmailCompletedSwitchOrder Class
 *
 * @method static WcEmailCompletedSwitchOrder get_instance()
 */
class WcEmailCompletedSwitchOrder extends BaseEmail {
    use SingletonTrait;

    protected function __construct() {
        $emails = \WC_Emails::instance()->get_emails();
        if ( ! isset( $emails['WCS_Email_Completed_Switch_Order'] ) ) {
            return;
        }
        $email            = $emails['WCS_Email_Completed_Switch_Order'];
        $this->id         = $email->id;
    ...
    public function get_default_elements() {
        $email_title = __( 'Subscription Switch Complete', 'woocommerce-subscriptions' );
        // translators: customer name.
        $email_text  = sprintf( esc_html__( 'Hi %s,', 'woocommerce' ), '[yaymail_billing_first_name] [yaymail_billing_last_name]' );
        $email_text .= '<br><br>' . esc_html__( 'You have successfully changed your subscription items. Your new order and subscription details are shown below for your reference:', 'woocommerce' );

        $new_subscriptions_detail_title = esc_html__( 'New subscription details', 'woocommerce-subscriptions' );
    ...
    public function get_template_path() {
        return YAYMAIL_ADDON_WS_PLUGIN_PATH . 'src/templates/emails/wc-email-completed-switch-order.php';
    }
}
