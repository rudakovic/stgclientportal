...
/**
 * WcEmailCustomerRenewalInvoice Class
 *
 * @method static WcEmailCustomerRenewalInvoice get_instance()
 */
class WcEmailCustomerRenewalInvoice extends BaseEmail {
    use SingletonTrait;

    protected function __construct() {
        $emails = \WC_Emails::instance()->get_emails();
        if ( ! isset( $emails['WCS_Email_Customer_Renewal_Invoice'] ) ) {
            return;
        }
        $email            = $emails['WCS_Email_Customer_Renewal_Invoice'];
        $this->id         = $email->id;
    ...
    public function get_default_elements() {
        $email_title = __( 'Invoice for renewal order', 'woocommerce-subscriptions' ) . '[yaymail_wc_subscription_id]';
                ...
                [
                    ...
                    'attributes' => [
                        'rich_text' => '<p><span>' . $email_text . '</span></p>
                                        <p><span>[yaymail_wc_subscription_renewal_invoice_text_detail]</span></p>',
                    ],
                ],
                [
                    'type'       => 'OrderDetails',
    ...
    public function get_template_path() {
        return YAYMAIL_ADDON_WS_PLUGIN_PATH . 'src/templates/emails/wc-email-customer-renewal-invoice.php';
    }
}
